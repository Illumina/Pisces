...
namespace VariantQualityRecalibration.Tests
{
    public class RecalTests
    {
        ...
        public void RecalibrateDirtyVcf()
        {
            ...
            var outFile = Path.Combine(TestPaths.LocalTestDataDirectory, "TestWithArtifacts.vcf.recal");
            var expectedFile = Path.Combine(TestPaths.LocalTestDataDirectory, "ExpectedDirty.vcf.recal");

            if (File.Exists(outFile))
                File.Delete(outFile);

            Logger.OpenLog(TestPaths.LocalScratchDirectory, "RecalibrateDirtyVcfLog.txt", true);
        ...
        public void RecalibrateCleanVcf()
        {
            ...
            var outFile = Path.Combine(TestPaths.LocalScratchDirectory, "Test.vcf.recal");
