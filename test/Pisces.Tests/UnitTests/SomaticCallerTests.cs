    ...
    public class SomaticCallerTests
    {
        ...
        private void ExecuteAndVerifyFlow(int numIterations, bool includeRefAlleles)
        {
            ...
            for (var i = 1; i <= numIterations; i++)
            {
                ...
                factory.MockStateManager.Verify(s => s.GetCandidatesToProcess(clearedPosition, _chrReference,It.IsAny<HashSet<Tuple<string,int,string,string>>>()), Times.Once);
            }
            factory.MockStateManager.Verify(s => s.GetCandidatesToProcess(null, _chrReference, It.IsAny<HashSet<Tuple<string, int, string, string>>>()), Times.Once);  // remainder
            ...
            mockWriter.Verify(w => w.WriteRemaining(It.IsAny<IRegionMapper>(),It.IsAny<Dictionary<int,List<Tuple<string,string>>>>()), Times.Exactly(1));  // final write
        }
