...
namespace StitchingLogic.Tests
{
    ...
    public class StitchingScenarioTests
    {
        ...
        public static IEnumerable<object[]> ScenarioData
        {
            get
            {
                var inputDirectory = TestPaths.SharedStitcherData;
                var outoutDirectory = TestPaths.LocalScratchDirectory;
                var visualFile = Path.Combine(outoutDirectory,"Stitching_Results.csv" + ".visuals.csv");
                var resultsFile = Path.Combine(outoutDirectory, "Stitching_Results.csv");
                if (File.Exists(visualFile))
                {
                    File.Delete(visualFile);
                }

                return StitchingScenarioParser.ParseScenariosFromDirectory(inputDirectory, resultsFile);
            }
        }
