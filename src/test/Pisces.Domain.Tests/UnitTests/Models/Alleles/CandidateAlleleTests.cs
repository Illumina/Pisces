...
namespace Pisces.Domain.Tests.UnitTests.Models.Alleles
{
    public class CandidateAlleleTests
    {
        ...
        public void Constructor_Undefined()
        {
            ...
            Assert.Equal(alternate, candidateVariant.AlternateAllele);
            //Verify Length
            Assert.Throws<ArgumentException>(() => candidateVariant.Length);
        ...
        {
            var allele = new CalledAllele() { ReferenceAllele = "A", AlternateAllele = "." };
            allele.SetType();
            Assert.Equal(AlleleCategory.Reference, allele.Type);

            allele = new CalledAllele() { ReferenceAllele = "A", AlternateAllele = "A" };
            allele.SetType();
            Assert.Equal(AlleleCategory.Reference, allele.Type);

            allele = new CalledAllele() { ReferenceAllele = "A", AlternateAllele = "C" };
            allele.SetType();
            Assert.Equal(AlleleCategory.Snv, allele.Type);

            allele = new CalledAllele() { ReferenceAllele = "AC", AlternateAllele = "CG" };
            allele.SetType();
            Assert.Equal(AlleleCategory.Mnv, allele.Type);

            allele = new CalledAllele() { ReferenceAllele = "AAA", AlternateAllele = "A" };
            allele.SetType();
            Assert.Equal(AlleleCategory.Deletion, allele.Type);

            allele = new CalledAllele() { ReferenceAllele = "A", AlternateAllele = "ACG" };
            allele.SetType();
            Assert.Equal(AlleleCategory.Insertion, allele.Type);

            allele = new CalledAllele() { ReferenceAllele = "AFGGGG", AlternateAllele = "ACG" };
            allele.SetType();
