...
namespace CallSomaticVariants.Tests.FunctionalTests
{
    public class NoCallFunctionalTests
    {
        ...
        private IVcfFileWriter GetMockWriter(List<BaseCalledAllele> calledAlleles)
        {
            var mockWriter = new Mock<IVcfFileWriter>();
            mockWriter.Setup(w => w.Write(It.IsAny<IEnumerable<BaseCalledAllele>>())).
                Callback((IEnumerable<BaseCalledAllele> alleles) => calledAlleles.AddRange(alleles));
        ...
        private void AddAlignmentSet(List<AlignmentSet> readSets, int copies, int read1Position, string read1Sequence, int read2Position, string read2Sequence)
        {
            for (var i = 0; i < copies; i++)
                readSets.Add(new AlignmentSet(
                    TestHelper.CreateRead(_chrName, read1Sequence, read1Position, new CigarAlignment("6M"), qualityForAll:30),
                    TestHelper.CreateRead(_chrName, read2Sequence, read2Position, new CigarAlignment("6M"), qualityForAll:30), 
                    true));
        }
    }
}
