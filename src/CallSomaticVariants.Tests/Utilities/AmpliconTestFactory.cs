...
namespace CallSomaticVariants.Test
{
    public class AmpliconTestFactory
    {
        ...
        {
            var reverseCigar = TestUtility.GetReadCigarFromStitched(stitchedCigar, readLength, true);
            ...
            var read1 = new BamAlignment()
            {
                RefID = 1,
                Position = ChrOffset,
                CigarData = TestUtility.GetReadCigarFromStitched(stitchedCigar, readLength, false),
                Bases = sequence.Substring(0, readLength),
                TagData = TestUtility.GetXCTagData(stitchedCigar),
