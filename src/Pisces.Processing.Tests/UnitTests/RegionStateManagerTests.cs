...
namespace Pisces.Processing.Tests.UnitTests
{
    public class BlockStateManagerTests
    {
        ...
        private void ExecuteAddAndGetCandidates(bool trackOpenEnded)
        {
            ...
            VerifyBatchContents(batch, candidates.Where(c => c.Coordinate <= 1000).ToList());
            ...
            VerifyBatchContents(batch, candidates.Where(c => c.Coordinate == 1001).ToList());
            ...
            VerifyBatchContents(batch, candidates.Where(c => c.Coordinate > 1001 && 
                (c.Coordinate <= 3000 || // properly from batch
                (trackOpenEnded &&
                ((c.Type == AlleleCategory.Mnv || c.Type == AlleleCategory.Snv) // OR collapsable from future batch (if collapsing is on)
                    && c.Coordinate + c.Alternate.Length - 1 <= 3003
                    && !c.OpenOnRight))     
                    )).ToList());
            ...
            VerifyBatchContents(batch, candidates.Where(c => c.Coordinate > 1001 && c.Coordinate < 4000 && c.Coordinate != 3002 || (!trackOpenEnded &&
                ((c.Type == AlleleCategory.Mnv || c.Type == AlleleCategory.Snv) 
                    && c.Coordinate + c.Alternate.Length - 1 <= 3003
                    && !c.OpenOnRight && c.Coordinate == 3002))     
                     ).ToList());
            ...
            VerifyBatchContents(batch, candidates.Where(c => c.Coordinate > 1001 && (!trackOpenEnded || c.Coordinate != 3002)).ToList());
