...
namespace Pisces.Processing.Tests.UnitTests
{
    public class BaseGenomeProcessorTests
    {
        ...
        private void ExecuteTest(int numberOfThreads, int expectedNumberOfThreads)
        {
            ...
            var logFullPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, logFile);

            if (File.Exists(logFullPath))
                File.Delete(logFullPath);

            Logger.TryOpenLog(AppDomain.CurrentDomain.BaseDirectory, logFile);
            ...
            using (var reader = new StreamReader(logFullPath))
            {
                string line;
