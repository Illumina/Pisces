...
namespace Common.IO.Utility
{
    public class Logger
    {
        ...
        private static bool _requireTokenForUnitTests = false;
        ...
        public static bool OpenLog(string logDir, string logFilePath, bool shouldLockForInstance = false)
        {
            ...
            //we want to InstanceLock the log, so only one unit test can open/close to the log 
            ...
            lock (typeof(Logger))
            {
                ...
                if (shouldLockForInstance)
                {
                    _requireTokenForUnitTests = true;
                }
            }
        ...
        public static bool CloseLog()
        {
            ...
            {
                Write("******************** Ending *********************");            
                _requireTokenForUnitTests = false;
                _unitTestToken.Set();
            }
            return true;
        }
