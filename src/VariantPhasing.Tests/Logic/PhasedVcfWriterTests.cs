...
namespace VariantPhasing.Tests.Logic
{
    ...
    {
        private string _outputFile = Path.Combine(UnitTestPaths.TestDataDirectory, "PhasedVcfWriterTest.Output.vcf");
        ...
        public void FilterHeader()
        {
            ...
            variants[0].Filters.AddRange(new List<FilterType> { FilterType.RMxN, FilterType.LowDepth, FilterType.LowVariantFrequency });
            variants[1].Filters.AddRange(new List<FilterType> { FilterType.IndelRepeatLength, FilterType.LowVariantQscore, FilterType.StrandBias });
        ...
        public void Write()
        {
            ...
            Assert.Throws<Exception>(() => writer.Write(new List<CalledAllele> { PhasedVariantTestUtilities.CreateDummyAllele("chr1", 123, "A", "G", 1000, 156) }));
            writer.Dispose();
