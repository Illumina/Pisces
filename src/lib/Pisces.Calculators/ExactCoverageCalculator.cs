...
namespace Pisces.Calculators
{
    public class ExactCoverageCalculator : CoverageCalculator
    {
        ...
        protected override void CalculateSpanning(CalledAllele variant, IAlleleSource alleleCountSource, int precedingPosition,
            int trailingPosition, bool isInsertion = false)
        {
            ...
            foreach (var spanningRead in spanningReads)
            {
                ...
                else
                {
                    ...
                    Read.UpdatePositionMap(spanningRead.ClipAdjustedStartPosition - (int)cigarData.GetPrefixClip(), cigarData, _positionMap, true, "SpanningReadSummary");
