...
namespace Gemini
{
    ...
    {
        T GetClassifierBlock(string name="Default");
    }
    ...
    public class ClassifierTransformerBlockFactory : ITransformerBlockFactory<TransformManyBlock<IEnumerable<ReadPair>, PairResult>>
    {
        ...
        public ClassifierTransformerBlockFactory(Dictionary<int, string> refIdMapping, StitcherOptions stitcherOptions, GeminiOptions geminiOptions, GeminiSampleOptions sampleOptions)
        {
        ...
        public TransformManyBlock<IEnumerable<ReadPair>, PairResult> GetClassifierBlock(string name="Default")
        {
            return GetPairClassifierBlock(name);
        }

        private TransformManyBlock<IEnumerable<ReadPair>, PairResult> GetPairClassifierBlock(string name="Default")
        {
            var readPair = new TransformManyBlock<IEnumerable<ReadPair>, PairResult>(
                (data) =>
                {
                    ...
                    var outFile = Path.Combine(_sampleOptions.OutputFolder,  Thread.CurrentThread.ManagedThreadId + "_" + name +"_softclips.csv");
                    Console.WriteLine($"Writing output to {outFile}");
                    using (var sw = new StreamWriter(outFile, true))
                    {
                        sw.WriteLine("#");
                        foreach (var kvp in classifier.SoftclipsEndingAtPosition)
                        {
                            var counts = new Dictionary<string, int[]>();
                            foreach (var seq in kvp.Value)
                            {
                                if (!counts.ContainsKey(seq))
                                {
                                    counts[seq] = new int[]{0,0};
                                }

                                counts[seq][0]++;
                                foreach (var shorterSeq in kvp.Value.Where(x => x.Length < seq.Length))
                                {
                                    if (seq.EndsWith(shorterSeq))
                                    {
                                        counts[seq][1]++;
                                    }
                                }
                            ...
                            var orderedCounts = counts.OrderByDescending(x => x.Key.Length)
                                .ThenByDescending(x => x.Value.Sum());
                            var topLongSeq = orderedCounts.First();

                            if (orderedCounts.Count() > 1 && orderedCounts.Skip(1).First().Value.Sum() > topLongSeq.Value.Sum() * 2)
                            {
                                topLongSeq = orderedCounts.Skip(1).First();
                            }
                            sw.WriteLine($"{kvp.Key},{kvp.Value.Count},{topLongSeq.Key},{kvp.Value.Distinct().Count()},{string.Join(";",orderedCounts.Select(x => $"{x.Key} ({string.Join("_",x.Value)})"))},End");
                            //Console.WriteLine($"SOFTCLIPS: {kvp.Key}, {kvp.Value.Count},{kvp.Value.OrderByDescending(x => x.Length).First()}");
                        }
                        foreach (var kvp in classifier.SoftclipsStartingAtPosition)
                        {
                            var counts = new Dictionary<string, int[]>();
                            foreach (var seq in kvp.Value)
                            {
                            ...
                            var frequencyCounts = counts.OrderByDescending(x => x.Value.Sum()).ToList();
                            var topFreq = frequencyCounts[0].Value.Sum();

                            sw.WriteLine($"{kvp.Key},{kvp.Value.Count},{topLongSeq.Key},{kvp.Value.Distinct().Count()},{string.Join(";", orderedCounts.Select(x => $"{x.Key} ({string.Join("_", x.Value)})"))},Start");

                            foreach (var count in orderedCounts)
                            {
                                if (count.Value.Sum() >= topFreq)
                                {
                                    sw.WriteLine(
                                        $"{kvp.Key},{kvp.Value.Count},{count.Key},{kvp.Value.Distinct().Count()},{string.Join(";", orderedCounts.Select(x => $"{x.Key} ({string.Join("_", x.Value)})"))},Start");
                                }
                            }

                            //if (topLongSeq.Value.Sum() == 1 && orderedCounts.Count() > 1)
                            //{
                            //    sw.WriteLine($"{kvp.Key},{kvp.Value.Count},{topLongSeq.Key},{kvp.Value.Distinct().Count()},{string.Join(";", orderedCounts.Select(x => $"{x.Key} ({string.Join("_", x.Value)})"))},Start");
                            //    topLongSeq = orderedCounts.Skip(1).First();
                            //}
