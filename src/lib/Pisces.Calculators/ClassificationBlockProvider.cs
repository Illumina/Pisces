...
namespace Gemini
{
    ...
    public class ClassificationBlockProvider : IClassificationBlockProvider
    {
        ...
        PairClassification[] classifications = new[]
        {
            ...
            PairClassification.Unhandleable 
        };
        ...
        public ClassificationBlockProvider(GeminiOptions geminiOptions, string chrom, ConcurrentDictionary<string, int> progressTracker, 
            ...
            IAggregateRegionProcessor aggregateRegionProcessor, bool lightDebug, PairResultBatchBlockFactory batchBlockFactory, IBinEvidenceFactory binEvidenceFactory, List<PairClassification> categoriesForRealignment, int maxDegreeOfParallelism, string outFolder = "")
        {
            _geminiOptions = geminiOptions;
            _chrom = chrom;
            ...
            var t = Task.WhenAll(allTasks)
                .ContinueWith(_ =>
                {
                    ...
                    var finalState = new RegionDataForAggregation()
                    {
                        ...
                        ConsistentSoftclips = GetPossibleSoftclipIndelsForRegion(_outFolder, $"{startPosition}")
                    };

                    //var softclipPossibleIndels = GetPossibleSoftclipIndelsForRegion();
                    //foreach (var candidate in softclipPossibleIndels)
                    //{
                    //    indelLookup.TryAdd(candidate.Key, new IndelEvidence(){Observations = candidate.Value, Forward = candidate.Value/2, Reverse = candidate.Value / 2, Quality = candidate.Value * 30, LeftAnchor = candidate.Value * 30, RightAnchor = candidate.Value * 30});
                    //}
        ...
        private static List<Tuple<int, string, int,bool>> GetPossibleSoftclipIndelsForRegion(string outfolder, string regionId)
        {
            var candidates = new List<Tuple<int, string, int,bool>>();

            foreach (var regionFile in Directory.EnumerateFiles(outfolder))
            {
                if (!regionFile.EndsWith($"_{regionId}_softclips.csv"))
                {
                    continue;
                }
                if (File.Exists(regionFile))
                {
                    var lines = File.ReadAllLines(regionFile);
                    foreach (var line in lines)
                    {
                        ...
                        var pos = int.Parse(splat[0]);
                        var longAlt = splat[2];
                        var support = int.Parse(splat[1]);
                        var numLengths = int.Parse(splat[3]);
                        var allClips = splat[4];
                        ...
                        candidates.Add(new Tuple<int, string, int, bool>(pos, longAlt, support, splat[5]=="Start"));
                    }
                }
            ...
            return candidates;
            //return new Dictionary<string, int>() {{"chr14:38210558 C>CGCTCTGAGCCCGGGCCACGCAGGG", 18}};
            //var candidates = new List<CandidateAllele>();
            //candidates.Add(new CandidateAllele("chr14", 38210558, "C", "CGCTCTGAGCCCGGGCCACGCAGGG",
