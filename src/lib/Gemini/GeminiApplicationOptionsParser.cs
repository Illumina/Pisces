...
namespace Gemini
{
    public class GeminiApplicationOptions : BaseApplicationOptions
    {
    ...
    public class GeminiApplicationOptionsParser : BaseOptionParser
    {

        public GeminiApplicationOptionsParser()
        {
            Options = new GeminiApplicationOptions();
        }

        public GeminiApplicationOptions ProgramOptions { get => (GeminiApplicationOptions)Options; }
        ...
        public override Dictionary<string, OptionSet> GetParsingMethods()
        {
            ...
            {
                // SUBPROCESS ONLY
                {"ChromRefId=", OptionTypes.INT + $" Chromosome RefId to filter on.", o => ProgramOptions.GeminiSampleOptions.RefId = int.Parse(o)},
...
// STITCHING + HYGEA OPTIONS CARRYING OVER
                ...
                {"DeferIndelStitch=", OptionTypes.BOOL + $" Whether to stitch non-disagreeing indel reads to start, or wait til realignment. Default: false.",  o=>  ProgramOptions.GeminiOptions.DeferStitchIndelReads = bool.Parse(o) },
                {"MaxNumDisagreeingStitchedBases=", OptionTypes.INT + $" Maximum number of stitched bases that can disagree between the two reads before a stitched read is rejected. Default: int.MaxValue.",  o=>  ProgramOptions.StitcherOptions.MaxNumDisagreeingBases = int.Parse(o) },
                ...
                // Subprocess only - for debugging purposes
                ...
                {"IntermediateDir=", OptionTypes.PATH + $" Intermediate directory to start from pre-categorized bams, for debugging. Default: none.",  o=>  ProgramOptions.GeminiSampleOptions.IntermediateDir = o },
                {"IndexPerChrom=", OptionTypes.BOOL + $" Whether to index the per-chromosome bams, for debugging. Default: false.",  o=>  ProgramOptions.GeminiOptions.IndexPerChrom = bool.Parse(o) },


                {"StitchOnly=", OptionTypes.BOOL + $" Whether to only perform stitching, skipping realignment. Default: false.",  o=>  ProgramOptions.GeminiOptions.StitchOnly = bool.Parse(o) },
                {"RealignOnly=", OptionTypes.BOOL + $" Whether to only perform realignment, skipping stitching. Default: false.",  o=>  ProgramOptions.GeminiOptions.SkipStitching = bool.Parse(o) },
