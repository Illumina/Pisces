...
namespace Gemini.Utility
{
    ...
    {
        public static bool MultiIndelContainsIndel(HashableIndel multiIndel, HashableIndel singleIndel)
        {
            ...
            var singleToString = HashableToString(singleIndel);
            if (multiIndel.OtherIndel == singleToString || HashableToString(multiIndel) == singleToString)
            {
                return true;
            }
        ...
        {
            return indel.Chromosome + ":" + indel.ReferencePosition + " " + indel.ReferenceAllele.Length + ">" + indel.AlternateAllele.Length;
        }
        ...
        public static CigarAlignment SoftclipCigar(CigarAlignment rawCigar, MatchType[] mismatchMap, uint originalSoftclipPrefix,
            uint originalSoftclipSuffix, bool rescueEdgeMatches = true, bool maskNsOnly = false, int prefixNs = 0, int suffixNs = 0, bool softclipEvenIfMatch = false)
        {
            ...
            for (var i = 0; i < maxSoftclipPrefixLength; i++)
            {
                ...
                {
                    shouldSoftclip = softclipEvenIfMatch || !rescueEdgeMatches || startedSoftclip || mismatchMap[mismatchMapIndex] != MatchType.Match;
                }
        ...
        public static MatchType[] GetMismatchMap(string readSequence, int[] positionMap, string refSequence, int startIndexInRefSequence = 0)
        {
            var mismatchMap = new MatchType[readSequence.Length];

            for (var i = 0; i < positionMap.Length; i++)
            {
                var position = positionMap[i] - startIndexInRefSequence;
        ...
        public static int? GetEditDistance(string readSequence, int[] positionMap, string refSequence, int startIndexInRefSequence = 0)
        {
        ...
        public static int GetSumOfMismatchQualities(byte[] quals, string readSequence, int[] positionMap, string refSequence,
        ...
        public static string NifyMismatches(string sequence, string otherSequence)
        {
