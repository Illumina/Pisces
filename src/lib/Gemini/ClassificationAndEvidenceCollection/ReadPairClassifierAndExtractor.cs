...
namespace Gemini.ClassificationAndEvidenceCollection
{
    public class ReadPairClassifierAndExtractor : IReadPairClassifierAndExtractor
    {
            ...
            int? numMismatchesInR1, int? numMismatchesInR2)
        {
            ...
            var r1Softclip = readPair.Read1.CigarData.HasSoftclips
                ? (readPair.Read1.CountBasesWithOperationType('S'))
                : 0;
            var r2Softclip = readPair.Read2.CigarData.HasSoftclips
                ? (readPair.Read2.CountBasesWithOperationType('S'))
                : 0;
