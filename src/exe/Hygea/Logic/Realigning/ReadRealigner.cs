...
namespace RealignIndels.Logic
{
    public class ReadRealigner
    {
        ...
        public ReadRealigner(AlignmentComparer comparer, bool tryThree = false, bool remaskSoftclips = true, bool maskPartialInsertion = false)
        {
            _tryThree = tryThree;
        ...
        private RealignmentResult RealignForAnchor(CandidateIndel[] indels, Read read, string refSequence, bool anchorOnLeft)
        {
            ...
            if (_maskPartialInsertion && hasInsertion)
            {
                ...
                for (int i = 0; i < result.Cigar.Count; i++)
                {
                    ...
                    else if (i==0 && result.Cigar[i].Type == 'I' && result.Cigar[i].Length < firstIndel.Length)
                    {
                        newCigar.Add(new CigarOp('S', result.Cigar[i].Length));
                    }
                    else if (i == result.Cigar.Count - 1 && result.Cigar[i].Type == 'I' && result.Cigar[i].Length < lastIndel.Length)
                    {
                        newCigar.Add(new CigarOp('S', result.Cigar[i].Length));
                    }
                    else
