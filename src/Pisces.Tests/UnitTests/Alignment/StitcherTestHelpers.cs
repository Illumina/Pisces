...
namespace Pisces.Tests.UnitTests.Alignment
{
    public static class StitcherTestHelpers
    {
        ...
        public static BaseStitcher GetStitcher(int minBaseCallQuality, bool xcStitcher = false, bool nifyDisagreements = false)
        {
            if (xcStitcher)
            {
                return new XCStitcher(minBaseCallQuality);
            }
            return new BasicStitcher(minBaseCallQuality, nifyDisagreements);
        }
