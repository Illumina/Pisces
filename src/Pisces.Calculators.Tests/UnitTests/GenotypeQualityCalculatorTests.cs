...
{
    public class GenotypeQualityTests
    {
        /// <summary>
        ...
        private static void TestCalculations(BaseCalledAllele variant, double depth, double[] testFrequencies, int[] expectedResults)
        {
            for (int i = 0; i < testFrequencies.Length; i++)
                TestCalculation(variant, testFrequencies[i], depth, expectedResults[i]);
        }

        private static void TestCalculation(BaseCalledAllele variant, double frequency, double depth, int expectedValue)
        {
            ...
            int result = GenotypeQualityCalculator.Compute(variant, 0, int.MaxValue);
